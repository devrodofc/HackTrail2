...
func _ready() -> void:
	...
	# MUDANÇA 1: Chama o diálogo de desfecho do Leozin
	EventBus.dialogue_requested.emit("conclusion_leozin")
...
func _on_speaker_changed(character_name: String) -> void:
	# Verifica se quem fala é o Leozin para dar foco a ele
	if character_name == "Leozin":
...
func _on_dialogue_finished() -> void:
	...
	if EventBus.speaker_changed.is_connected(_on_speaker_changed):
		...
		print("[CONCLUSÃO] Caso encerrado. Indo para a escolha final...")
		
	get_tree().change_scene_to_file("res://gameplay/lore/final_scene/make_your_choice.tscn")
